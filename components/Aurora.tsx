...
function AuroraComponent({
  ...
  useEffect(() => {
    ...
    const initialStops = (colorStops || []).map((hex) => {
    ...
    const update = (t: number) => {
      ...
      const tBase = (current.time ?? t * 0.01) * (current.speed ?? 1.0);
      program.uniforms.uTime.value = tBase * 0.1;
...
export default React.memo(AuroraComponent);
